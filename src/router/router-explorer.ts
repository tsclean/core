...
export class RouterExplorer {
  ...
  constructor (
    ...
    const pipesContextCreator = new HandlersContextCreator(container, config)
    const pipesConsumer = new HandlersConsumer()
    const guardsContextCreator = new AccessResourceContextCreator(
      container,
      config
    )
    const guardsConsumer = new AccessResourceConsumer()
  ...
  private applyHostFilter (
    ...
    return <T extends Record<string, any> = any, R = any>(
      req: T,
      res: R,
      next: () => void
    ) => {
      ...
      for (const exp of hostRegExps) {
        ...
        if (match) {
          exp.keys.forEach(
            (key: any, i: any) => (req.hosts[key.name] = match[i + 1])
          )
  ...
  public createRequestScopedHandler (
    ...
    return async <T extends Record<any, any>, R>(
      req: T,
      res: R,
      next: () => void
    ) => {
