...
import {isFunction, isNil, isObject, isString, isUndefined, clc} from "../../utils";
import {
    OPTIONAL_DEPS_METADATA, OPTIONAL_PROPERTY_DEPS_METADATA,
    PARAMTYPES_METADATA,
...
export class Injector {
    ...
    private printFoundInModuleLog(token: InstanceTokenType, moduleRef: Module): void {
        if (!Injector.isDebugMode()) {
            return;
        }
        const tokenName = Injector.getTokenName(token);
