...
export class DependenciesScanner {
    ...
    public async scanForModules(moduleDefinition: ForwardReferenceInterface | Type<unknown> | DynamicModuleInterface | Promise<DynamicModuleInterface>,
                                scope: Type<unknown>[] = [], ctxRegistry: (ForwardReferenceInterface | DynamicModuleInterface | Type<unknown>)[] = []): Promise<Module[]> {

        const moduleInstance = await this.insertModule(moduleDefinition, scope);
        moduleDefinition =
            moduleDefinition instanceof Promise ? await moduleDefinition : moduleDefinition;

        ctxRegistry.push(moduleDefinition);

        if (this.isForwardReference(moduleDefinition))
            moduleDefinition = (moduleDefinition as ForwardReferenceInterface).forwardRef();

        const modules = !this.isDynamicModule(moduleDefinition as Type<any> | DynamicModuleInterface)
        ...
        for (const [index, innerModule] of modules.entries()) {
            ...
            const moduleRefs = await this.scanForModules(
                innerModule, [].concat(scope, moduleDefinition), ctxRegistry);
    ...
    public reflectInjectables(component: Type<InjectableType>, token: string, metadataKey: string) {
