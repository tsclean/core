...
import { addLeadingSlash, isFunction, isString } from '../utils/shared.utils'
...
export const mapToClass = <T extends Function | Type<any>>(
  middleware: T,
  excludedRoutes: ExcludeRouteMetadataInterface[],
  httpAdapter: HttpServer
) => {
...
export function isMiddlewareRouteExcluded (
  ...
  const pathname =
    queryParamsIndex >= 0 ? originalUrl.slice(0, queryParamsIndex) : originalUrl
